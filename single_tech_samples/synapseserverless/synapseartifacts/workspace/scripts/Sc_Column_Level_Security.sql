...
/* Step 3: Log in into Azure with  USER_2 */
/* Step 4: Navigate to Synapse Studio: https://web.azuresynapse.net/ and make sure you are logged with USER_2)*/
/* Step 5: Run the following query*/
select airport_fee from VW_202010
/* Step 6: You should receive the following result: */
/* The SELECT permission was denied on the column 'airport_fee' of the object 'VW_202010', database 'db_serverless', schema 'dbo'.*/

/* Step 7: Run the following query on the Synapse Studio logged with USER_1  */
GRANT SELECT ON VW_202010(VendorID,total_amount) TO SynapseDbUser;

/* Step 8: Run the following query on the Synapse Studio logged with USER_2  */
select VendorID,total_amount from VW_202010

/* Step 9: You should be able to see the results as the AAD group is authorized on those 2 columns  */